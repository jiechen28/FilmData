...
for i in range(0,9,1):
    loop_file = pd.read_csv(os.path.join(os.path.dirname(__file__), "title_level_df_{}.csv").format(i))
    title_df = pd.concat([title_df,loop_file]).reset_index(drop=True)
...
title_df.numVotes = title_df.numVotes.astype(int, errors = 'ignore')
...
def displaygraph(title):
    ...
    try:
        ...
        if req.status_code==200:
            ...
            return vote_dis_fig, age_vote_dis_fig, link1, 'The average weighted vote score is: {}. The total vote count is {:,}.'.format(avg_score,total_cnt)
        else:
            return empty_fig,empty_fig, html.P('Oops, no movie poster found!'),html.P('Opps no movie info found!')
    except:
        return empty_fig,empty_fig, html.P('Oops, no movie poster found!'),  html.P('Opps no movie info found!')
